...
\question{
  Dans la station les spationautes ont un poids $P_\text{ISS} = m \times g_\text{ISS}$.
  Calculer la valeur de $g_\text{ISS}$ sachant que
  \begin{equation*}
    g_\text{ISS} =  G \times \dfrac{\MTerre}{(\RTerre + h)^2}
...
\question{
  ...
  En utilisant le principe d'inertie et en justifiant clairement, donner la relation entre $\vv{F}_\inertie$ et $\vv{P}$.\competence{APP, ANA/RAI}
}{
...
\question{
  ...
  \begin{equation*}
    F_\inertie = m \times \dfrac{v^2}{\RTerre + h}
  ...
  D'après le principe d'inertie, $P = F_\intertie$, donc
  \begin{align*}
    m \times g &= m \times \dfrac{v^2}{\RTerre + h} \\
    m \times v^2 &= m \times g \times (\RTerre + h) \\
    v^2 &= g \times (\RTerre + h) \\
    v &= \sqrt{g \times (\RTerre + h)} \\
    &= \sqrt{\qty{9.81}{\m\per\s\squared} \times (\num{6,37e6} + \qty{4,14e5}{\m})} \\
    &= \qty{8.16}{\m\per\s}
  \end{align*}
  On retrouve presque la même valeur que celle fournie dans l'énoncé, cette valeur est donc cohérente.
  \points{3}
}
