\vspace*{-8pt}
\titreSection{Sang et anémie}\points{12}
\vspace*{-4pt}
...
\newcommand{\sangCentrifugeuse}[3]{
  \begin{tikzpicture}
    ...
    \tikzPhaseBasTubeEssais[couleur= red-400,    largeur= 0.35cm, hauteur= #1cm]
    \tikzPhaseTubeEssais   [couleur= white,      largeur= 0.35cm, hauteur= #1cm, phase= #2cm]
    \tikzPhaseTubeEssais   [couleur= yellow-300, largeur= 0.35cm, hauteur= #2cm, phase= #3cm]
    \tikzTubeEssais[hauteur = 2.75cm, largeur = 0.35cm]
    % Légende
    \tikzVecteur (0.45, #3 - 0.1)  (1.1, 0) {Plasma}[right]
    \tikzVecteur (0.45, #2 - 0.08) (1.1, 0) {Globules blancs}[right]
    \tikzVecteur (0.45, -0.1)      (1.1, 0) {Globules rouges}[right]
  \end{tikzpicture}
}

\begin{doc}{Composition du sang et anémie}[\label{doc:composition_sang}]
  ...
  \separationBlocs{
    ...
    \sangCentrifugeuse{0.8}{1}{1.75}
    
    \legende{Échantillon de sang d'une personne normale.}
  }[0.48]{
    \centering \hspace{40pt}
    \sangCentrifugeuse{0.3}{0.5}{1.75}
...
%
\question{
  Indiquer en justifiant si le contenu des tubes à essais du document~\ref{doc:composition_sang} 
...
\question{
  En observant les échantillons de sang, déduire le constituant du sang qui contient l'hémoglobines.\competence{APP}
}{
...
%
\pasCorrection{ \newpage \vspace*{-40pt} }
\begin{doc}{Dosage de l'hémoglobine}[\label{doc:dosage_hemoglobine}]
...
\question{
  En utilisant le document~\ref{doc:dosage_hemoglobine}, indiquer en justifiant la concentration en hémoglobine de l'échantillon de sang.\competence{APP, VAL}
}{
