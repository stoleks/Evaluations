...
\begin{doc}{Forces de frottements}
  ...
  \begin{equation*}
    f = \dfrac{1}{2} \times k \times S \times v^2
    % f = \dfrac{1}{2} \times \rho \times C_x \times S \times v^2
  \end{equation*}
  Avec :
  \begin{listePoints}
    \item $k = \variationSujet{\num{0.246}}{\num{0.185}}\;\unit{\newton\s\squared\per\m\tothe{4}}$ un coefficient de frottement ;
...
\begin{doc}{Ballon de football}
  ...
  On cherche à étudier les forces qui s'exercent sur un ballon de football pendant un penalty, d'abords quand il est posé au sol, puis quand le tir est effectué.
...
\question{
  Indiquer quel est le système étudié et donner un référentiel approprié pour analyser son mouvement.\competence{APP}
}{
  On étudie le mouvement du ballon de football, dans le référentiel terrestre.\points{1}
}

\question{
  Citer la ou les forces qui s’exercent sur le ballon quand il est posé au sol, puis calculer leurs valeurs.\competence{APP, REA}
}{
...
\question{
  ...
  Il y a toujours le poids $\vv{P}$ et il y a les frottements de l'air $\vv{f}$.
  \points{1}
}
...
\question{
  Indiquer, en justifiant, si Le ballon de football a un mouvement rectiligne uniforme lorsqu’il est en l’air.\competence{VAL, ANA/RAI}
}{
  Les forces ne se compensent pas, donc d'après le principe d'inertie le ballon n'a pas un mouvement rectiligne uniforme.\points{1}
}
...
\question{
  ...
  \begin{equation*}
    f = \dfrac{1}{2}k S v^2
    = \dfrac{1}{2} \times
    \times \variationSujet{\num{0,246}}{\num{0,185}}\; \unit{\newton\s\squared\per\m\tothe{4}}
    \times \qty{0.16}{\m\squared}
    \times \left(\variationSujet{\qty{50}{\m\per\s}}{\qty{45}{\m\per\s}}\right)^2
    = \variationSujet{\qty{49,2}{\newton}}{\qty{30,0}{\newton}}
    \points{3}
  \end{equation*}
}
