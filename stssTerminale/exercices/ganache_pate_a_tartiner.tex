...
\begin{doc}{Les oméga 3 et 6}[\label{doc:DC1_omega_3_6}]
...
\question{
  ...
  Un acide gras est une molécule avec un groupe carboxyle \chemfig{COOH} lié à une longue chaîne carbonée.\points{0.5}

  Un triglycéride est une molécule de glycérol estérifié avec trois acide gras.\points{0.5}
}

\question{
  Nommer les molécules désignées par A et B dans l'équation de la réaction d'hydrolyse de l'oléine.
}{
...
\question{
  Calculer la masse d'oléine $m_\text{oléine}$ présente dans \qty{100}{\g} d'huile de palme.
}{
  On a une masse d'oléine $m_\text{oléine} = \qty{38,2}{\percent} \times \qty{100}{\g} = \qty{38,2}{\g}$, \points{1}
}

\question{
  En déduire la quantité de matière $n_\text{oléine}$ d'oléine.
}{

  La quantité de matière vaut donc
  \begin{equation*}
...
\question{
  Calculer la masse d'huile de palme qu'il faudrait manger pour respecter cet apport. Est-ce une valeur raisonnable pour une alimentation saine ?
}{
