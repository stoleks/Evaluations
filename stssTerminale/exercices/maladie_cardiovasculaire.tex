...
\begin{doc}{Maladie cardiovasculaire}[\label{doc:E3_cardiovasculaire}]
  \extrait{
...
\begin{doc}{Principe de l'échographie Doppler}[\label{doc:E3_doppler}]
...
\begin{doc}{Le décalage Doppler $\Delta f$}[\label{doc:E3_decalage_doppler}]
...
\question{
  ...
  \begin{align*}
    \Delta f &= \dfrac{2f_e v}{c} \\
    v \dfrac{2f_e}{c} &= \Delta f \\
    v &= \Delta f \dfrac{c}{2f_e}
  \end{align*}
}
...
\question{
  ...
  \begin{equation*}
    v = \Delta f \dfrac{c}{2 f_e}
    = \qty{2,1e3}{\hertz} \times \dfrac{\qty{1540}{\m\per\s}}{2 \times \qty{4,5e6}{\hertz}}
    = \qty{0.36}{\m\per\s}
  \end{equation*}
}
...
\question{
  ...
  Dans l'artère $v = \qty{0.36}{\m\per\s} = \qty{36}{\cm\per\s}$, la vitesse d'écoulement est inférieure à la vitesse normale et on a un ralentissement de la circulation sanguine, ce qui est un symptôme de l'athérosclérose.
}

\question{
  Donner les conséquences d'une athérosclérose.
}{
  Une athérosclérose peut entraîner un accident cardiovasculaire (infarctus du myocarde, accident vasculaire cérébral...).
}